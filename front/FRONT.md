...
Шапка: логотип, Главная (`/`), Каталог (`/catalog`), Контакты, иконки избранного и корзины (счётчики), на мобиле — burger. Поиска в шапке и в каталоге нет. На мобилке логотип только внизу burger-sheet по центру (широкий wordmark не влезает рядом с иконками); клик по нему ведёт на `/`.
...
1. Hero: `src/components/home/HomeHero.tsx` — «Премиальные автозапчасти», схема тормозного диска, сетка, CTA «Перейти в каталог» (`/catalog`).
2. Как заказать — `HowToOrder` (lazy). Три шага — непрозрачные карточки `bg-card` (`src/components/home/HowToOrder.tsx`).
3. Доставка / гарантия / оплата — `Assurances` (lazy).

Каталог живёт на `/catalog`, не на главной.
...
- `< 768`: 2 колонки запчастей (карточка компактная), марки — 1 ряд, модели — 2 ряда, горизонтальный скролл, шапка → sheet.
...
2. [x] TanStack Router + layout (шапка/подвал/мобиле) + пустые страницы.
   ...
   - `src/main.tsx` — `RouterProvider` вместо `App.tsx` (файл удалён); `defaultPendingMs: 300` и `defaultPendingMinMs: 0`, иначе клик по марке/модели сразу сносит каталог в скелетон; запасной скелетон, пока loader роута без своего `pendingComponent`
   ...
   - `src/components/layout/Header.tsx` — лого (`hidden md:flex`), Каталог/Контакты, избранное, корзина, burger
   - `src/components/layout/Footer.tsx`, `MobileNav.tsx` — лого внизу sheet по центру, ссылка на `/`
...
3. [x] Главная на API: ленты, фильтры в search-params, сетка, пустые состояния.
   ...
   - категории: подсказка «Выберите категорию»; если больше 10, в конце чип «Показать все» (`ShowAll.tsx`, лимит в `lib/catalog-strip.ts`); клик раскрывает полный список с `flex-wrap`, «Свернуть» у заголовка
...
13. [x] Страница запчасти + галерея.
    - `src/routes/parts.$partId.tsx` — `/parts/{id}-{slug}`, loader `sparePartsQueries.detail`, 404 → `notFound`; `PartPagePending` (`ProductPageSkeleton`), pending по `defaultPendingMs`
