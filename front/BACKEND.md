...
XML-карта сайта: `/`, `/catalog`, `/catalog/:mark`, `/catalog/:mark/:model`, `/contacts` и все `/parts/:id-slug` с `lastmod` и фото. Роботы заходят по `public/sitemap.xml` (индекс) или сразу на этот URL. `FRONTEND_ORIGIN` без слэша в конце — база для `<loc>`.
...
| 400 | не удалось сжать фото | `Не удалось обработать изображение` |
...
jpeg/png/webp при сохранении сжимаются в WebP: полный файл до 1600px (`/uploads/<dir>/<uuid>.webp`) и превью до 800px (`…/<uuid>.thumb.webp`). В сущности хранится только путь к полному файлу. GIF не трогаем (анимация). Удаление полного файла стирает и превью.

В сущности: `images: string[]` — пути вида `/uploads/marks/<uuid>.webp`.  
Показ: карточки — `imageThumbSrc(path)` (`src/lib/images.ts`), галерея — полный путь. Старые jpeg/png/gif без превью открываются как есть.
